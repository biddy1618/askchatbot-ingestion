...
def embed_data(df, max_seq_size):
    ...
    '''
    logger.info(f'STARTING TRANSFORMING - MAX_SEQ_SIZE - {max_seq_size}')
    df_texts = []
    ...
    df['texts'] = df_texts
    logger.info(f'The number of vectors to be ingested: {len([item["vector"] for row in df["texts"] for item in row])}', end='\n\n')
